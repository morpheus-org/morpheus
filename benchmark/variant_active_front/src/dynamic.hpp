...
using Hyb_lf = morpheus::hyb_matrix<long long, float, morpheus::host_memory>;
	...
	#elif NUM_TYPES == 6
		using format = boost::variant<Coo_id,Csr_id,Dia_id,Ell_id,Hyb_id,Dense_id>;
	#elif NUM_TYPES == 12
		using format = boost::variant<Coo_id,Csr_id,Dia_id,Ell_id,Hyb_id,Dense_id,
									Coo_if,Csr_if,Dia_if,Ell_if,Hyb_if,Dense_if>;
	#elif NUM_TYPES == 20
		using format = boost::variant<Coo_id,Csr_id,Dia_id,Ell_id,Hyb_id,Dense_id,
									Coo_if,Csr_if,Dia_if,Ell_if,Hyb_if,Dense_if,
									Coo_ld,Csr_ld,Dia_ld,Ell_ld,Hyb_ld,
									Coo_lf,Csr_lf,Dia_lf>;
	#endif
#else
	#include <variant>
	#if NUM_TYPES == 1
	...
	#elif NUM_TYPES == 6
		using format = std::variant<Coo_id,Csr_id,Dia_id,Ell_id,Hyb_id,Dense_id>;
	#elif NUM_TYPES == 12
		using format = std::variant<Coo_id,Csr_id,Dia_id,Ell_id,Hyb_id,Dense_id,
									Coo_if,Csr_if,Dia_if,Ell_if,Hyb_if,Dense_if>;
	#elif NUM_TYPES == 20
