...
public:
  ...
  inline CooMatrix(const index_type num_rows, const index_type num_cols,
      ...
      : _m(num_rows), _n(num_cols), _nnz(num_entries), row_indices(num_entries),
        column_indices(num_entries), values(num_entries) {}

  // Construct from another matrix type
  template <typename MatrixType> CooMatrix(const MatrixType &matrix) {
