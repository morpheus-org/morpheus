...
#ifndef MORPHEUS_HDC_COPY_IMPL_HPP
#define MORPHEUS_HDC_COPY_IMPL_HPP
...
void copy(const SourceType& src, DestinationType& dst,
  ...
  MORPHEUS_ASSERT(dst.dia().nnnz() + dst.csr().nnnz() ==
                      src.cdia().nnnz() + src.ccsr().nnnz(),
                  "Destination matrix must have equal number of non-zeros to "
                  "the source matrix");

  Morpheus::Impl::copy(src.cdia(), dst.dia());
  Morpheus::Impl::copy(src.ccsr(), dst.csr());
}

}  // namespace Impl
}  // namespace Morpheus

#endif  // MORPHEUS_HDC_COPY_IMPL_HPP