...
#ifndef MORPHEUS_DIA_CUDA_NON_ZEROS_PER_ROW_IMPL_HPP
#define MORPHEUS_DIA_CUDA_NON_ZEROS_PER_ROW_IMPL_HPP
...
#include <impl/Dia/Kernels/Morpheus_MatrixAnalytics_Impl.hpp>
...
inline void count_nnz_per_row(
  ...
  const size_type MAX_BLOCKS = max_active_blocks(
      Kernels::count_nnz_per_row_dia_kernel<size_type, index_type, value_type,
                                            BLOCK_SIZE>,
  ...
  Kernels::count_nnz_per_row_dia_kernel<size_type, index_type, value_type,
      ...
      <<<NUM_BLOCKS, BLOCK_SIZE, 0>>>(A.nrows(), A.ncols(), num_diagonals,
                                      pitch, D, nnz_per_row_ptr);

#if defined(DEBUG) || defined(MORPHEUS_DEBUG)
  getLastCudaError("count_nnz_per_row_dia_kernel: Kernel execution failed");
#endif
}
...
#endif  // MORPHEUS_DIA_CUDA_NON_ZEROS_PER_ROW_IMPL_HPP