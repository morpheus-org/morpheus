...
void multiply_armpl_coo(const size_t M, const size_t N, const size_t nnnz, 
                  const IndexType* rind, const IndexType* cind, 
                  const double* vals, const double* x, double* y, bool init){
  armpl_spmat_t armpl_mat;
  double beta = init ? 0.0 : 1.0;

  armpl_status_t info = armpl_spmat_create_coo_d(&armpl_mat, M, N, nnnz, rind, cind, vals, ARMPL_SPARSE_CREATE_NOCOPY);
...
void multiply_armpl_coo(const size_t M, const size_t N, const size_t nnnz, 
  ...
  armpl_status_t info = armpl_spmat_create_coo_s(&armpl_mat, M, N, nnnz, rind, cind, vals, ARMPL_SPARSE_CREATE_NOCOPY);
  if (info!=ARMPL_STATUS_SUCCESS)
