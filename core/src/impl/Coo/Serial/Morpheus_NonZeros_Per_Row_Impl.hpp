...
#ifndef MORPHEUS_COO_SERIAL_NON_ZEROS_PER_ROW_IMPL_HPP
#define MORPHEUS_COO_SERIAL_NON_ZEROS_PER_ROW_IMPL_HPP
...
inline void count_nnz_per_row(
  ...
  if (init) {
    for (size_type n = 0; n < A.nrows(); n++) {
      nnz_per_row[n] = value_type(0);
    }
  }

  for (size_type n = 0; n < A.nnnz(); n++) {
    nnz_per_row[A.crow_indices(n)]++;
  }
}
...
#endif  // MORPHEUS_COO_SERIAL_NON_ZEROS_PER_ROW_IMPL_HPP