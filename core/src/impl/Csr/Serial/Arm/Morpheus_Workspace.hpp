...
#define MORPHEUS_CSR_ARM_WORKSPACE_HPP
...
class ArmPLCsrWorkspace_Serial {
 public:
  ...
  armpl_spmat_t handle(armpl_int_t m, armpl_int_t n, const armpl_int_t *row_ptr,
                       const armpl_int_t *col_indx, const ValueType *vals,
                       armpl_int_t flags) {
    armpl_int_t *key = (armpl_int_t *)row_ptr;
    if (_handles_map.find(key) == _handles_map.end()) {
      armpl_spmat_t armpl_mat;
      armpl_spmat_create_csr<ValueType>(&armpl_mat, m, n, row_ptr, col_indx,
                                        vals, flags);
