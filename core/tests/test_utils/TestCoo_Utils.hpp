...
#ifndef MORPHEUS_CORE_TEST_COO_UTILS_HPP
#define MORPHEUS_CORE_TEST_COO_UTILS_HPP

#include <Morpheus_Core.hpp>

template <typename Container>
void test_traits(Morpheus::CooTag) {
...
void check_shapes(const T1& A, const T2& A_mirror, Morpheus::CooTag) {
  ...
  ASSERT_EQ(A.crow_indices().size(), A.nnnz());
  ASSERT_EQ(A.ccolumn_indices().size(), A.nnnz());
  ASSERT_EQ(A.cvalues().size(), A.nnnz());
  ASSERT_EQ(A.crow_indices().size(), A_mirror.crow_indices().size());
