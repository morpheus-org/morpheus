...
if [ "$#" -lt 6 ]; then
    ...
    if [ -z "$6" ]; then
        FILENAME="timings"
        echo -e "\tFilename::       spmv-$FILENAME-$MACHINE-$COMPILER-$DATASET-$TARGET.csv"
    fi
fi
...
RESULTS_FILE="$ROOT_PATH/core/benchmarks/results/processed/$EXPERIMENT/spmv-$FILENAME-$MACHINE-$COMPILER-$DATASET-$TARGET.csv"
